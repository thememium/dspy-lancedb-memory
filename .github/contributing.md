# DSPy LanceDB Memory Contributing Guide

Hi! I'm really excited that you are interested in contributing to DSPy LanceDB Memory. Before submitting your contribution, please make sure to take a moment and read through the following guidelines:
...
This is equivalent to `uv run python -c "from dspy_lancedb_memory import memory; print(memory)"`.
...
# Run a specific test
$ uv run pytest tests/test_store.py::test_create_memory -v
```
...
```
├── src/dspy_lancedb_memory/    # Source code
│   ├── __init__.py             # Public API exports
│   ├── config.py               # Global configuration (memory.configure())
│   ├── extraction.py           # DSPy ChainOfThought memory extraction
│   ├── memory.py               # SDK module — configure() and Store()
│   ├── models.py               # Pydantic models (MemoryItem, MemoryType)
│   ├── reranking.py            # LiteLLMReranker for cross-encoder reranking
│   └── store.py                # Core Store class — CRUD, upsert, search
├── tests/                      # Test files
...
- Use absolute imports from the `dspy_lancedb_memory` package for local modules
...
from dspy_lancedb_memory.models import MemoryItem, MemoryType
...
Thank you to all the people who have already contributed to DSPy LanceDB Memory!

<a href="https://github.com/thememium/dspy-lancedb-memory/graphs/contributors"><img src="https://contrib.rocks/image?repo=thememium/dspy-lancedb-memory" /></a>